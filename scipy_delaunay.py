...
while True:  # The main loop, this will run forever until stopped manually.
    ...
    for simplex in simplexVs:  # iterate through every triangle
        ...
        draw.polygon([tuple(x) for x in polycopy],
                     fill=g.color_rgb(pixelsg.getPixel(int(ax), int(ay))[0], pixelsg.getPixel(int(ax), int(ay))[1],
                                      pixelsg.getPixel(int(ax), int(ay))[2]))
        triangle.draw(win)
